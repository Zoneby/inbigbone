...
mc.init = function() {
  ...
  createjs.Sound.registerSounds([
    ...
    {id:'r05',src:app.resPath+"media/r05.mp3"},
    {id:'r06',src:app.resPath+"media/r06.mp3"},
    {id:'r10',src:app.resPath+"media/r10.mp3"},
    {id:'r11',src:app.resPath+"media/r11.mp3"},
    {id:'r21',src:app.resPath+"media/r21.mp3"},
    {id:'r28',src:app.resPath+"media/r28.mp3"},
    {id:'r31',src:app.resPath+"media/r31.mp3"},
    {id:'r32',src:app.resPath+"media/r32.mp3"},
    ...
    {id:'r59',src:app.resPath+"media/r59.mp3"},
  ]);
}
  ...
  ss["atlas_s2_"] = queue.getResult("atlas_s2_");
	...
	setTimeout(function (){
		...
		setTimeout(function (){
			...
			mc.scale = mc.sceneCanvas.height/ma.getBounds().height;
			ma.scaleX = mc.scale;
			ma.scaleY = mc.scale;
...
mc.initDrag = function (){
	// var wraperH = mc.winH - mc.xsboxH + 5;
	//初始相对视角位置
	// ma.x = -2300;
	...
	ma.addEventListener('mousedown',function (e){
		if (!app.canTouchMove){
			// createjs.Tween.pause();
			app.moveTween.setPaused(true);
			app.canTouchMove = true;
		}
	...
	ma.addEventListener('pressmove',function (e){
		...
		if (x>0){
			x = 0;
		}else if (x < -ma.getBounds().width*mc.scale+mc.sceneCanvas.width){
			x = -ma.getBounds().width*mc.scale+mc.sceneCanvas.width
		}
		// if (y>0){
		// 	y = 0;
...
mc.initListener = function (){
	...
	ma.r05.addEventListener('click',function (e){
		e.currentTarget.play();
		createjs.Sound.play('r05');
	});
	ma.r06.addEventListener('click',function (e){
		e.currentTarget.play();
		createjs.Sound.play('r06');
	});
	...
	ma.r59.addEventListener('click',function (e){
		createjs.Sound.play('r59');
	})
}