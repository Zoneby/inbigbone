...
app.btnStart = function (){
	...
	try{
		shareData.timelineTitle = shareList[app.clueIndex].timelineTitle[0];
		shareData.imgUrl = shareList[app.clueIndex].shareImgUrl;
...
app.sceneStartMove = function (){
	...
	// 	// app.initClueSlider();
	// }.bind(this));
	app.moveTween = createjs.Tween.get(ma).to({x:-2000},3000).call(function (){
		$('.swiper-wrapper').addClass('ani');
		app.canTouchMove = !0;
		// app.initClueSlider();
	}.bind(this));
...
app.loadAniStart = function (){
	function makeEl(){
		var i = parseInt(13*Math.random())+1;
		var randomTop = parseInt(220*Math.random())*5+'px';
		var duration = (parseInt(30*Math.random())+40)/10;
		var elStr = '<i class="ld t'+i+'" style="left:100%;top:'+randomTop+';-webkit-animation-duration:'+duration+'s"></i>';
		// return $(elStr);
		var el = $(elStr);
		el.on('webkitAnimationEnd',function (){$(this).remove()})
		$('#loading').append(el);
	}

	app.loadTimer = setInterval(makeEl,100);
...
$(function (){
	...
	$('.suc_btn').on('click',function (){
		...
		try{
			shareData.timelineTitle = shareList[app.clueIndex].timelineTitle[1];
			mobileShare(shareData);
		}catch(e){}
	})
